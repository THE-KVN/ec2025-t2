...
namespace NETCoreBot.Strategy
{
    public static class Cobra
    {
        ...
        public static int VISITED_TILE_PENALTY_FACTOR = 1;
        public static int REVERSING_TO_PARENT_PENALTY = 4;
        public static double ZOOKEEPER_AVOIDANCE_FACTOR = 20;
        public static int ENEMY_PATH_AVOIDANCE = 2;
        public static int PELLET_BONUS = 1;
        public static int DANGER_THRESHOLD = 200;
        public static int MAX_PELLET_CANDIDATES = 4; // Number of pellets to consider for multi-pellet pathfinding
        ...
        public static bool IsPathStillValid(List<CobraNode>? path, Animal myAnimal, List<Cell> allCells)
        {
        ...
        public static BotCommand? EscapeZookeeperV2(Animal myAnimal)
        {
            ...
            // --- Zookeeper direction relative to myAnimal ---
            var zookeeperDir = GetDirectionV2(closestZookeeper.X, closestZookeeper.Y, myAnimal.X, myAnimal.Y);
            ...
            {
                List<(Cell Portal, int Score)> candidates = new();

                foreach (var portal in allPortalCellsOrdered)
                {


                    var portalDir = GetDirectionV2(myAnimal.X, myAnimal.Y, portal.X, portal.Y);

                    // Danger score: penalize portals toward zookeeper
                    int danger = portalDir == zookeeperDir ? 1000 : 0;
                    int distance = Manhattan(myAnimal.X,myAnimal.Y, portal.X,portal.Y);
                    int score = distance + danger;

                    candidates.Add((portal, score));
                }
                ...
                {
                    portalTarget = candidates.OrderBy(c => c.Score).First().Portal;
                }
             }
            ...
            double scoreLeft = PELLETS_LEFT / 20;
        ...
        public static BotCommand? EvaluateAndRunThroughPortal(Animal myAnimal)
        {
            ...
            // Evaluate pellets
            int thisPellets = GAME_STATE.Cells.Count(c => c.Content == CellContent.Pellet && isThisSide(c)) + 30;
        ...
        public static (Cell? bestPellet, List<CobraNode>? bestPath) FindBestClusterMultiPelletPath(Animal myAnimal)
        {
        ...
        public static (Cell? bestPellet, List<CobraNode>? bestPath) FindBestClusterMultiPelletPathV2(Animal myAnimal)
        {
            // Build a grid for cell lookup.
            var grid = GAME_STATE.Cells.ToDictionary(c => (c.X, c.Y), c => c.Content);
            ...
            var (clusterCandidate, bestCluster) = FindPelletInLargestConnectedClusterWeightedV2(myAnimal, GAME_STATE.Cells, ALPHA);
        ...
        public static (Cell? bestPellet, List<Cell>? bestCluster) FindPelletInLargestConnectedClusterWeightedV2(Animal myAnimal, List<Cell> allCells, double alpha)
        {
            ...
            foreach (var pcell in pelletCells)
            {
                ...
                if (!visited.Contains(pos))
                {
                    var cluster = BFSClusterV2(pos, grid, visited);
                    clusters.Add(cluster);
                }
            }
        ...
        public static List<Cell> BFSClusterV2((int x, int y) start, Dictionary<(int, int), CellContent> grid, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static (Cell? bestPellet, List<CobraNode>? bestPath) FindBestMultiPelletPath(
            Animal myAnimal,
            Dictionary<(int, int), CellContent> grid)
        {
    ...
    int maxPelletsToChain)
        ...
        public static List<CobraNode>? ValidateOrComputePath(Animal myAnimal, Cell target, Dictionary<(int, int), CellContent> grid)
        {
        ...
        public static List<CobraNode>? FindPath(
            int startX,
            int startY,
            int targetX,
            int targetY,
        ...
        public static IEnumerable<CobraNode> GetNeighborsPortal(
            CobraNode current,
            Dictionary<(int, int), CellContent> grid,
            int targetX,
            int targetY,
        ...
        public static HashSet<(int, int)> PredictContestedPellets(Animal myAnimal)
        {
            ...
            foreach (var animal in GAME_STATE.Animals)
            {
                ...
                var path = FindPathOpponent(animal.X, animal.Y, pellet.X, pellet.Y, grid, animal);
                if (path != null && path.Count > 1)
                {
        ...
        public static List<CobraNode>? FindPathOpponent(
            int startX,
            int startY,
            int targetX,
            int targetY,
        ...
        public static IEnumerable<CobraNode> GetNeighborsPortalOpponent(
                CobraNode current,
                Dictionary<(int, int), CellContent> grid,
                int targetX,
                int targetY,
        ...
        public static BotAction? ComputeNextMoveRateLimited(Animal myAnimal, List<CobraNode> path)
        {
            if (path.Count == 0)
            {
        ...
        private static BotAction GetDirectionV2(int fromX, int fromY, int toX, int toY)
        {
            ...
            if (Math.Abs(dx) > Math.Abs(dy))
                return dx > 0 ? BotAction.Right : BotAction.Left;
            else
                return dy > 0 ? BotAction.Down : BotAction.Up;
        }
        ...
        private static int CountPelletsOnPath(List<CobraNode> path, Dictionary<(int, int), CellContent> grid)
        {
            int count = 0;
            foreach (var node in path)
            {
        ...
        public static void LogCurrentPath(List<CobraNode> path)
        {
    ...
    public class CobraNode
    {
        ...
        public CobraNode(int x, int y, int g, int h, CobraNode? parent, int tieBreak = 0)
        {
            X = x;
            Y = y;
            G = g;

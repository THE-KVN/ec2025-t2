...
namespace NETCoreBot.Strategy
{
    public static class Cobra
    {
        ...
        public static List<CobraNode>? PersistentPath { get; set; } = null;
        ...
        public static CobraCluster BestCluster { get; set; }
            ...
            Dictionary<(int, int), CellContent> grid)
        {
            ...
            var openSet = new List<CobraNode>();
            var closedSet = new HashSet<(int, int)>();

            var startNode = new CobraNode(
                startX, startY,
                0,
        ...
        public static IEnumerable<CobraNode> GetNeighborsZookeeper(
            CobraNode current,
            Dictionary<(int, int), CellContent> grid,
            int targetX,
            int targetY)
        ...
        public static CobraCluster FindBestClusterEarlyGame()
        {
            ...
            var visited = new HashSet<(int, int)>();
            var clusters = new List<CobraCluster>();
            ...
            CobraCluster bestCluster = clusters.OrderByDescending(c => c.Score).FirstOrDefault();
            ...
            List<CobraNode>? bestPath = FindPath(ME.X, ME.Y, bestPellet.X, bestPellet.Y); ;
            ...
            var fullPath = new List<CobraNode>();
            ...
            CobraNode finalPellet = fullPath.Last();
        ...
        public static CobraCluster BFSClusterEarlyGame((int x, int y) start, HashSet<(int, int)> visited)
        {
        ...
        public static CobraCluster FindBestClusterMidGame()
        {
        ...
        public static CobraCluster BFSClusterMidGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static List<CobraNode> FindLongestPelletPathInCluster((int x, int y) start, HashSet<(int, int)> clusterSet)
        {
            var bestPath = new List<CobraNode>();
            var visited = new HashSet<(int, int)>();

            // 1. Does this cluster contain any power pellet at all?
            bool clusterHasPower = clusterSet.Any(c => IsPowerPellet(c.Item1, c.Item2));

            void DFS((int x, int y) current, List<CobraNode> currentPath, bool hasPowerInPath)
            {
                visited.Add(current);
                ...
                currentPath.Add(new CobraNode(current.x, current.y, 0, 0, null, 0, 0, 0, null));
                ...
                {
                    bestPath = new List<CobraNode>(currentPath);
                }
            ...
            DFS(start, new List<CobraNode>(), false);
        ...
        public static CobraCluster FindBestClusterLateGame()
        {
            var pelletCells = GAME_STATE.Cells.Where(c => c.Content == CellContent.Pellet && !CONTESTED_CELLS_MAP.Contains(new(c.X, c.Y)) && !CORRIDOR_CELLS_MAP.Contains(new(c.X, c.Y))).ToList();
        ...
        public static CobraCluster BFSClusterLateGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static List<CobraNode>? FindPath(int startX, int startY, int targetX, int targetY, bool ignorePortals = false)
        {
            // ─── data-structures ──────────────────────────────────────────────────────
            var open = new PriorityQueue<CobraNode, int>();               // min-heap by F
            var openDict = new Dictionary<(int, int), CobraNode>();           // coord → best node
        ...
        public static IEnumerable<CobraNode> FindPathNeighbors(CobraNode current, int targetX, int targetY, bool ignorePortals = false)
        {
            // (dx, dy, heading)
            ...
            foreach (var (dx, dy, dir) in dirs)
            {
                ...
                // 2.  bonus / penalty
                //--------------------------------------------------------------------
                int bonus = FindPathBonus(new CobraNode(nx, ny, newG, 0, current, 0, nextBlank, nextStreak, null));
                int penalty = FindPathPenalty(current, new CobraNode(nx, ny, newG, 0, current, 0, nextBlank, nextStreak, null));
        ...
        public static int FindPathBonus(CobraNode node)
        {
            int bonus = 0;
        ...
        public static int FindPathPenalty(CobraNode current, CobraNode neighbor)
        {
            int penalty = 0;
        ...
        public static List<CobraNode> ReconstructPath(CobraNode node)
        {
        ...
        public static IEnumerable<CobraNode> GetNeighborsClean(CobraNode current, int targetX, int targetY)
        {
            int[,] directions = new int[,] { { 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 } };
        ...
        public static double EvaluatePathScore(List<CobraNode> path)
        {
            int pellets = CountPelletsOnPath(path);
            return pellets;
        }
        public static int CountPelletsOnPath(List<CobraNode> path)
        {
            int count = 0;
        ...
        public static int CountPowerPelletsOnPath(List<CobraNode> path)
        {
            int count = 0;
            foreach (var node in path)
            {
        ...
        public static List<CobraNode>? FindPathOpponent(int startX, int startY, int targetX, int targetY)
        {
            var openSet = new List<CobraNode>();
            var closedSet = new HashSet<(int, int)>();
        ...
        public static IEnumerable<CobraNode> FindPathNeighborsOpponent(CobraNode current, int targetX, int targetY)
        {
            int[,] directions = new int[,] { { 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 } };
            ...
            for (int i = 0; i < directions.GetLength(0); i++)
            {
                ...
                int newG = current.G + 1;
                int bonus = FindPathBonusOpponent(new CobraNode(nx, ny, newG, 0, current, 0, 0, 0, null));
                int penalty = 0;
        ...
        public static int FindPathBonusOpponent(CobraNode node)
        {
            int bonus = 0;
        ...
        private static readonly Dictionary<Guid, CobraZookeeperTracker> _zkCache = new(); // NEW
        ...
        private static int KeeperStepsToMe(Guid zkId)
        {
            ...
            if (!_zkCache.TryGetValue(zkId, out var trk))
                _zkCache[zkId] = trk = new CobraZookeeperTracker();
        ...
        private static List<CobraNode>? PathToAnimal(Zookeeper zk, Animal tgt)
            => FindPathKeeper(zk.X, zk.Y, tgt.X, tgt.Y);

        // ---------------------------------------------------------------- A*
        private static List<CobraNode>? FindPathKeeper(int sx, int sy, int tx, int ty)
        {
            var open = new PriorityQueue<CobraNode, int>();
            var best = new Dictionary<(int, int), int>();

            var start = new CobraNode(sx, sy, 0,
        ...
        private static IEnumerable<CobraNode> KeeperNeighbours(CobraNode n, int tx, int ty)
        {
            foreach (var (dx, dy) in new (int, int)[] { (0, -1), (0, 1), (-1, 0), (1, 0) })
            {
                ...
                yield return new CobraNode(nx, ny, n.G + 1,
        ...
        /// </summary>
        private static bool PathsIntersect(List<CobraNode> pathA, List<CobraNode> pathB)
        {
        ...
        public static BotCommand? EscapeViaPortalMultiZK()
        {
            ...
            var zkPathsToMe = new Dictionary<Guid, List<CobraNode>>();
    ...
    public class CobraNode
    {
        ...
        public CobraNode? Parent;
        public int BlankStreak;
        public int PelletStreak;
        public BotAction? DirFromParent { get; }

        public CobraNode(int x, int y, int g, int h, CobraNode? parent, int tieBreak, int blankStreak, int pelletStreak, BotAction? dirFromParent)
        {
            X = x;
            Y = y;
            G = g;
    ...
    public class CobraCluster
    {
        public List<Cell> Cells { get; set; }
        public float Score { get; set; }
        public Cell EntryPoint { get; set; }
        public List<CobraNode>? Path { get; set; }
    }

    public sealed class CobraZookeeperTracker
    {
        public Animal? Target;
        public List<CobraNode>? Path;

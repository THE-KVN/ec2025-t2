...
namespace NETCoreBot.Strategy
{
    public static class Elephant
    {
        ...
        public static ElephantCluster BestCluster { get; set; }
        ...
        public static List<ElephantNode>? FindPathZookeeper(
            int startX,
            int startY,
            int targetX,
            int targetY,
        ...
        public static IEnumerable<ElephantNode> GetNeighborsZookeeper(
        ...
        public static ElephantCluster FindBestClusterEarlyGame()
        {
            var pelletCells = GAME_STATE.Cells.Where(c => c.Content == CellContent.Pellet || c.Content == CellContent.PowerPellet).ToList();

            if (pelletCells.Count == 0)
                return null;

            var visited = new HashSet<(int, int)>();
            var clusters = new List<ElephantCluster>();
            ...
            ElephantCluster bestCluster = clusters.OrderByDescending(c => c.Score).FirstOrDefault();
            ...
            List<ElephantNode>? bestPath = FindPath(ME.X, ME.Y, bestPellet.X, bestPellet.Y); ;
        ...
        public static ElephantCluster BFSClusterEarlyGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            ...
            var result = new ElephantCluster()
            {
                Cells = cluster,
                Score = 0,
                EntryPoint = null
        ...
        public static ElephantCluster FindBestClusterMidGame()
        {
        ...
        public static ElephantCluster BFSClusterMidGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static List<ElephantNode> FindLongestPelletPathInCluster((int x, int y) start, HashSet<(int, int)> clusterSet)
        {
            ...
            void DFS((int x, int y) current, List<ElephantNode> currentPath, bool hasPowerInPath)
            {
                visited.Add(current);
                ...
                currentPath.Add(new ElephantNode(current.x, current.y, 0, 0, null, 0, 0, 0, null));
            ...
            DFS(start, new List<ElephantNode>(), false);
        ...
        public static ElephantCluster FindBestClusterLateGame()
        {
            var pelletCells = GAME_STATE.Cells.Where(c => c.Content == CellContent.Pellet && !CONTESTED_CELLS_MAP.Contains(new(c.X, c.Y)) && !CORRIDOR_CELLS_MAP.Contains(new(c.X, c.Y))).ToList();
        ...
        public static ElephantCluster BFSClusterLateGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static List<ElephantNode>? FindPath(int startX, int startY, int targetX, int targetY, bool ignorePortals = false)
        {
            // ─── data-structures ──────────────────────────────────────────────────────
            var open = new PriorityQueue<ElephantNode, int>();               // min-heap by F
            var openDict = new Dictionary<(int, int), ElephantNode>();           // coord → best node
        ...
        public static IEnumerable<ElephantNode> FindPathNeighbors(ElephantNode current, int targetX, int targetY, bool ignorePortals = false)
        {
            // (dx, dy, heading)
            ...
            foreach (var (dx, dy, dir) in dirs)
            {
                ...
                // 2.  bonus / penalty
                //--------------------------------------------------------------------
                int bonus = FindPathBonus(new ElephantNode(nx, ny, newG, 0, current, 0, nextBlank, nextStreak, null));
                int penalty = FindPathPenalty(current, new ElephantNode(nx, ny, newG, 0, current, 0, nextBlank, nextStreak, null));
        ...
        public static int FindPathBonus(ElephantNode node)
        {
            int bonus = 0;
        ...
        public static int FindPathPenalty(ElephantNode current, ElephantNode neighbor)
        {
            int penalty = 0;
        ...
        public static List<ElephantNode> ReconstructPath(ElephantNode node)
        {
        ...
        public static IEnumerable<ElephantNode> GetNeighborsClean(ElephantNode current, int targetX, int targetY)
        {
            int[,] directions = new int[,] { { 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 } };
        ...
        public static int CountPelletsOnPath(List<ElephantNode> path)
        {
            int count = 0;
        ...
        public static int CountPowerPelletsOnPath(List<ElephantNode> path)
        {
            int count = 0;
            foreach (var node in path)
            {
        ...
        public static List<ElephantNode>? FindPathOpponent(int startX, int startY, int targetX, int targetY)
        {
            var openSet = new List<ElephantNode>();
            var closedSet = new HashSet<(int, int)>();
        ...
        public static IEnumerable<ElephantNode> FindPathNeighborsOpponent(ElephantNode current, int targetX, int targetY)
        {
            int[,] directions = new int[,] { { 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 } };
            ...
            for (int i = 0; i < directions.GetLength(0); i++)
            {
                ...
                int newG = current.G + 1;
                int bonus = FindPathBonusOpponent(new ElephantNode(nx, ny, newG, 0, current, 0, 0, 0, null));
                int penalty = 0;
        ...
        public static int FindPathBonusOpponent(ElephantNode node)
        {
            int bonus = 0;
        ...
        private static readonly Dictionary<Guid, ElephantZookeeperTracker> _zkCache = new(); // NEW
        ...
        private static int KeeperStepsToMe(Guid zkId)
        {
            ...
            if (!_zkCache.TryGetValue(zkId, out var trk))
                _zkCache[zkId] = trk = new ElephantZookeeperTracker();
        ...
        private static List<ElephantNode>? PathToAnimal(Zookeeper zk, Animal tgt)
            => FindPathKeeper(zk.X, zk.Y, tgt.X, tgt.Y);

        // ---------------------------------------------------------------- A*
        private static List<ElephantNode>? FindPathKeeper(int sx, int sy, int tx, int ty)
        {
            var open = new PriorityQueue<ElephantNode, int>();
            var best = new Dictionary<(int, int), int>();

            var start = new ElephantNode(sx, sy, 0,
        ...
        private static IEnumerable<ElephantNode> KeeperNeighbours(ElephantNode n, int tx, int ty)
        {
            foreach (var (dx, dy) in new (int, int)[] { (0, -1), (0, 1), (-1, 0), (1, 0) })
            {
                ...
                yield return new ElephantNode(nx, ny, n.G + 1,
        ...
        /// </summary>
        private static bool PathsIntersect(List<ElephantNode> pathA, List<ElephantNode> pathB)
        {
        ...
        public static BotCommand? EscapeViaPortalMultiZK()
        {
            ...
            var zkPathsToMe = new Dictionary<Guid, List<ElephantNode>>();
        ...
        public static float ScoreCluster(ElephantCluster cluster)
        {
    ...
    public class ElephantNode
    {
        ...
        public ElephantNode(int x, int y, int g, int h, ElephantNode? parent, int tieBreak, int blankStreak, int pelletStreak, BotAction? dirFromParent)
        {
            X = x;
            Y = y;
            G = g;
    ...
    public class ElephantCluster
    {
        public List<Cell> Cells { get; set; }
        public float Score { get; set; }
        public Cell EntryPoint { get; set; }
        public List<ElephantNode>? Path { get; set; }
    }

    public sealed class ElephantZookeeperTracker
    {
        public Animal? Target;
        public List<ElephantNode>? Path;

...
namespace NETCoreBot.Strategy
{
    public static class Gorilla
    {
        ...
        public static List<GorillaNode>? PersistentPath { get; set; } = null;
        ...
        public static GorillaCluster BestCluster { get; set; }
        ...
        //new
        private const int MAX_CLUSTER_PELLETS_LIMIT_N = 75;
        private const int MAX_CLUSTER_SIZE_N = 75;     // keep existing semantics
        private const float ALPHA_N = 0.75f;  // tune as before
        ...
        public static BotCommand? EscapeZookeeperEvent()
        {
            ...
            if (IsZookeeperNear())
            {
                ...
                if (ME.X == ME.SpawnX && ME.Y == ME.SpawnY)
                {
                    ...
                    return FinaliseAction(null);// Already at spawn, no need to escape
                }
        ...
        public static List<GorillaNode>? FindPathZookeeper(
            int startX,
            int startY,
            int targetX,
            int targetY,
            Dictionary<(int, int), CellContent> grid)
        {
            ...
            var openSet = new List<GorillaNode>();
            var closedSet = new HashSet<(int, int)>();

            var startNode = new GorillaNode(
                startX, startY,
                0,
        ...
        public static IEnumerable<GorillaNode> GetNeighborsZookeeper(
            GorillaNode current,
            Dictionary<(int, int), CellContent> grid,
            int targetX,
            int targetY)
        ...
        public static GorillaCluster FindBestClusterEarlyGame()
        {
            ...
            var visited = new HashSet<(int, int)>();
            var clusters = new List<GorillaCluster>();
            ...
            GorillaCluster bestCluster = clusters.OrderByDescending(c => c.Score).FirstOrDefault();
            ...
            List<GorillaNode>? bestPath = FindPath(ME.X, ME.Y, bestPellet.X, bestPellet.Y); ;
            ...
            var fullPath = new List<GorillaNode>();
        ...
        public static GorillaCluster BFSClusterEarlyGame((int x, int y) start, HashSet<(int, int)> visited)
        {
        ...
        public static GorillaCluster FindBestClusterMidGame()
        {
            ...
            List<GorillaNode>? bestPath = null;
        ...
        public static GorillaCluster BFSClusterMidGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static List<GorillaNode> FindLongestPelletPathInCluster((int x, int y) start, HashSet<(int, int)> clusterSet)
        {
            var bestPath = new List<GorillaNode>();
            var visited = new HashSet<(int, int)>();

            // 1. Does this cluster contain any power pellet at all?
            bool clusterHasPower = clusterSet.Any(c => IsPowerPellet(c.Item1, c.Item2));

            void DFS((int x, int y) current, List<GorillaNode> currentPath, bool hasPowerInPath)
            {
                visited.Add(current);
                ...
                currentPath.Add(new GorillaNode(current.x, current.y, 0, 0, null, 0, 0, 0, null));
                ...
                {
                    bestPath = new List<GorillaNode>(currentPath);
                }
            ...
            DFS(start, new List<GorillaNode>(), false);
        ...
        public static GorillaCluster FindBestClusterLateGame()
        {
            var pelletCells = GAME_STATE.Cells.Where(c => c.Content == CellContent.Pellet && !CONTESTED_CELLS_MAP.Contains(new(c.X, c.Y)) && !CORRIDOR_CELLS_MAP.Contains(new(c.X, c.Y))).ToList();
        ...
        public static GorillaCluster BFSClusterLateGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static List<GorillaNode>? FindPath(int startX, int startY, int targetX, int targetY, bool ignorePortals = false)
        {
            // ─── data-structures ──────────────────────────────────────────────────────
            var open = new PriorityQueue<GorillaNode, int>();               // min-heap by F
            var openDict = new Dictionary<(int, int), GorillaNode>();           // coord → best node
        ...
        public static IEnumerable<GorillaNode> FindPathNeighbors(GorillaNode current, int targetX, int targetY, bool ignorePortals = false)
        {
            // (dx, dy, heading)
            ...
            foreach (var (dx, dy, dir) in dirs)
            {
                ...
                // 2.  bonus / penalty
                //--------------------------------------------------------------------
                int bonus = FindPathBonus(new GorillaNode(nx, ny, newG, 0, current, 0, nextBlank, nextStreak, null));
                int penalty = FindPathPenalty(current, new GorillaNode(nx, ny, newG, 0, current, 0, nextBlank, nextStreak, null));
        ...
        public static int FindPathBonus(GorillaNode node)
        {
            int bonus = 0;
        ...
        public static int FindPathPenalty(GorillaNode current, GorillaNode neighbor)
        {
            int penalty = 0;
        ...
        public static List<GorillaNode> ReconstructPath(GorillaNode node)
        {
            var path = new List<GorillaNode>();
            while (node != null)
            {
                path.Add(node);
                node = node.Parent;
        ...
        public static IEnumerable<GorillaNode> GetNeighborsClean(GorillaNode current, int targetX, int targetY)
        {
            int[,] directions = new int[,] { { 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 } };
        ...
        public static double EvaluatePathScore(List<GorillaNode> path)
        {
            int pellets = CountPelletsOnPath(path);
            return pellets;
        }
        public static int CountPelletsOnPath(List<GorillaNode> path)
        {
            int count = 0;
        ...
        public static List<GorillaNode>? FindPathOpponent(int startX, int startY, int targetX, int targetY)
        {
            var openSet = new List<GorillaNode>();
            var closedSet = new HashSet<(int, int)>();
        ...
        public static IEnumerable<GorillaNode> FindPathNeighborsOpponent(GorillaNode current, int targetX, int targetY)
        {
            int[,] directions = new int[,] { { 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 } };
            ...
            for (int i = 0; i < directions.GetLength(0); i++)
            {
                ...
                int newG = current.G + 1;
                int bonus = FindPathBonusOpponent(new GorillaNode(nx, ny, newG, 0, current, 0, 0, 0, null));
                int penalty = 0;
        ...
        public static int FindPathBonusOpponent(GorillaNode node)
        {
            int bonus = 0;
        ...
        private static readonly Dictionary<Guid, GorillaZookeeperTracker> _zkCache = new(); // NEW
        ...
        private static int KeeperStepsToMe(Guid zkId)
        {
            ...
            if (!_zkCache.TryGetValue(zkId, out var trk))
                _zkCache[zkId] = trk = new GorillaZookeeperTracker();
        ...
        private static List<GorillaNode>? PathToAnimal(Zookeeper zk, Animal tgt)
            => FindPathKeeper(zk.X, zk.Y, tgt.X, tgt.Y);

        // ---------------------------------------------------------------- A*
        private static List<GorillaNode>? FindPathKeeper(int sx, int sy, int tx, int ty)
        {
            var open = new PriorityQueue<GorillaNode, int>();
            var best = new Dictionary<(int, int), int>();

            var start = new GorillaNode(sx, sy, 0,
        ...
        private static IEnumerable<GorillaNode> KeeperNeighbours(GorillaNode n, int tx, int ty)
        {
            foreach (var (dx, dy) in new (int, int)[] { (0, -1), (0, 1), (-1, 0), (1, 0) })
            {
                ...
                yield return new GorillaNode(nx, ny, n.G + 1,
        ...
        /// </summary>
        private static bool PathsIntersect(List<GorillaNode> pathA, List<GorillaNode> pathB)
        {
        ...
        public static BotCommand? EscapeViaPortalMultiZK()
        {
            ...
            var zkPathsToMe = new Dictionary<Guid, List<GorillaNode>>();
        ...
        public static float ScoreCluster(GorillaCluster cluster)
        {
    ...
    public class GorillaNode
    {
        ...
        public GorillaNode? Parent;
        public int BlankStreak;
        public int PelletStreak;
        public BotAction? DirFromParent { get; }

        public GorillaNode(int x, int y, int g, int h, GorillaNode? parent, int tieBreak, int blankStreak, int pelletStreak, BotAction? dirFromParent)
        {
            X = x;
            Y = y;
            G = g;
    ...
    public class GorillaCluster
    {
        public List<Cell> Cells { get; set; }
        public float Score { get; set; }
        public Cell EntryPoint { get; set; }
        public List<GorillaNode>? Path { get; set; }
    }

    public sealed class GorillaZookeeperTracker
    {
        public Animal? Target;
        public List<GorillaNode>? Path;

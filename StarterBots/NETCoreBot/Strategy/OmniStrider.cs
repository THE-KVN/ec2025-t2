...
{
    public static class OmniStrider
    {
        #region Variables
        ...
        public static List<OmniStriderNode>? PersistentPath { get; set; } = null;
        ...
        public static OmniStriderCluster BestCluster { get; set; }

        private const int PELLET_MARGIN = 60;   // pellets needed to outweigh flip-flop
        private const int PORTAL_COOLDOWN = 10;   // ticks to stay put after crossing
        private static int _portalLockUntilTick = -1;
        ...
        public static BotCommand? ProcessState(GameState gameStateDTO, Guid id)
        {
            ...
            using (GameLogger.Benchmark("[Event] Respawning"))
            {
                var respawnResult = RespawnEvent();
                if (respawnResult == null)
                {
            ...
            //using (GameLogger.Benchmark("[Event] Escaping Zookepers"))
            //{
        ...
        public static BotCommand? PortalEvalEvent()
        {
            ...
            // 0. cool-down guard
            //-----------------------------------------------------------------------
            if (GAME_STATE.Tick < _portalLockUntilTick)
            ...
            _portalLockUntilTick = GAME_STATE.Tick + PORTAL_COOLDOWN;
        ...
        //                var escape = EscapeViaPortal(); // your existing routine – unchanged
        //                if (escape != null)
        ...
        public static List<OmniStriderNode>? FindPathZookeeper(
            int startX,
            int startY,
            int targetX,
            int targetY,
            Dictionary<(int, int), CellContent> grid)
        {
            ...
            var openSet = new List<OmniStriderNode>();
            var closedSet = new HashSet<(int, int)>();

            var startNode = new OmniStriderNode(
                startX, startY,
                0,
        ...
        public static IEnumerable<OmniStriderNode> GetNeighborsZookeeper(
            OmniStriderNode current,
            Dictionary<(int, int), CellContent> grid,
            int targetX,
            int targetY)
        {
            ...
            for (int i = 0; i < directions.GetLength(0); i++)
            {
                ...
                yield return new OmniStriderNode(
                    nx, ny,
                    newG, newH,
                    current,
                    0,
        ...
        public static float ScoreCluster(OmniStriderCluster cluster)
        {
        ...
        public static OmniStriderCluster FindBestClusterEarlyGame()
        {
            ...
            var clusters = new List<OmniStriderCluster>();
            ...
            OmniStriderCluster bestCluster = clusters.OrderByDescending(c => c.Score).FirstOrDefault();
            ...
            List<OmniStriderNode>? bestPath = null;
        ...
        public static OmniStriderCluster BFSClusterEarlyGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            ...
            var result = new OmniStriderCluster()
            {
                Cells = cluster,
                Score = 0,
                EntryPoint = null
        ...
        public static OmniStriderCluster FindBestClusterMidGame()
        {
            ...
            var fullPath = new List<OmniStriderNode>();
        ...
        public static OmniStriderCluster BFSClusterMidGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static List<OmniStriderNode> FindLongestPelletPathInCluster((int x, int y) start, HashSet<(int, int)> clusterSet) // ✅ Tune this externally
        {
            var bestPath = new List<OmniStriderNode>();
            var visited = new HashSet<(int, int)>();

            void DFS((int x, int y) current, List<OmniStriderNode> currentPath)
            {
                visited.Add(current);
                currentPath.Add(new OmniStriderNode(current.x, current.y, 0, 0, null, 0,0,0,null));

                // ✅ Always track the longest path seen so far
                if (currentPath.Count > bestPath.Count)
                    bestPath = new List<OmniStriderNode>(currentPath);
            ...
            DFS(start, new List<OmniStriderNode>());
        ...
        public static OmniStriderCluster FindBestClusterLateGame()
        {
            var pelletCells = GAME_STATE.Cells.Where(c => c.Content == CellContent.Pellet  && !CONTESTED_CELLS_MAP.Contains(new (c.X,c.Y)) && !CORRIDOR_CELLS_MAP.Contains(new (c.X, c.Y))).ToList();
        ...
        public static OmniStriderCluster BFSClusterLateGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static List<OmniStriderNode>? FindPath(int startX, int startY, int targetX, int targetY)
        {
            // ─── data-structures ──────────────────────────────────────────────────────
            var open = new PriorityQueue<OmniStriderNode, int>();               // min-heap by F
            var openDict = new Dictionary<(int, int), OmniStriderNode>();           // coord → best node
            ...
            while (open.TryDequeue(out var current, out _))
            {
                ...
                // ─── neighbours ──────────────────────────────────────────────────────
                foreach (var neighbour in FindPathNeighbors(current, targetX, targetY))
                {
        ...
        public static IEnumerable<OmniStriderNode> FindPathNeighbors(OmniStriderNode current,int targetX,int targetY)
        {
            // (dx, dy, heading)
            ...
            foreach (var (dx, dy, dir) in dirs)
            {
                ...
                // 2.  bonus / penalty
                //--------------------------------------------------------------------
                int bonus = FindPathBonus(new OmniStriderNode(nx, ny, newG, 0, current, 0, nextBlank, nextStreak, null));
                int penalty = FindPathPenalty(current, new OmniStriderNode(nx, ny, newG, 0, current, 0, nextBlank, nextStreak, null));
        ...
        public static int FindPathBonus(OmniStriderNode node)
        {
            int bonus = 0;
        ...
        public static int FindPathPenalty(OmniStriderNode current, OmniStriderNode neighbor)
        {
            int penalty = 0;
        ...
        public static List<OmniStriderNode> ReconstructPath(OmniStriderNode node)
        {
            var path = new List<OmniStriderNode>();
            while (node != null)
            {
                path.Add(node);
                node = node.Parent;
        ...
        public static IEnumerable<OmniStriderNode> GetNeighborsClean(OmniStriderNode current, int targetX, int targetY)
        {
            int[,] directions = new int[,] { { 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 } };
        ...
        public static double EvaluatePathScore(List<OmniStriderNode> path)
        {
            int pellets = CountPelletsOnPath(path);
            return pellets;
        }
        public static int CountPelletsOnPath(List<OmniStriderNode> path)
        {
            int count = 0;
        ...
        public static List<OmniStriderNode>? FindPathOpponent(int startX, int startY, int targetX, int targetY)
        {
            var openSet = new List<OmniStriderNode>();
            var closedSet = new HashSet<(int, int)>();
        ...
        public static IEnumerable<OmniStriderNode> FindPathNeighborsOpponent(OmniStriderNode current, int targetX, int targetY)
        {
            int[,] directions = new int[,] { { 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 } };
            ...
            for (int i = 0; i < directions.GetLength(0); i++)
            {
                ...
                var neighbor = new OmniStriderNode(
                    nx,
                    ny,
                    0,
                    0,
        ...
        private static readonly Dictionary<Guid, OmniStriderZookeeperTracker> _zkCache = new(); // NEW
        ...
        private static int KeeperStepsToMe(Guid zkId)
        {
            ...
            if (!_zkCache.TryGetValue(zkId, out var trk))
                _zkCache[zkId] = trk = new OmniStriderZookeeperTracker();
        ...
        private static List<OmniStriderNode>? PathToAnimal(Zookeeper zk, Animal tgt)
            => FindPathKeeper(zk.X, zk.Y, tgt.X, tgt.Y);

        // ---------------------------------------------------------------- A*
        private static List<OmniStriderNode>? FindPathKeeper(int sx, int sy, int tx, int ty)
        {
            var open = new PriorityQueue<OmniStriderNode, int>();
            var best = new Dictionary<(int, int), int>();

            var start = new OmniStriderNode(sx, sy, 0,
        ...
        private static IEnumerable<OmniStriderNode> KeeperNeighbours(OmniStriderNode n, int tx, int ty)
        {
            foreach (var (dx, dy) in new (int, int)[] { (0, -1), (0, 1), (-1, 0), (1, 0) })
            {
                ...
                yield return new OmniStriderNode(nx, ny, n.G + 1,
        ...
        public static BotCommand? EscapeViaPortal()
        {
            ...
            // 4. Compute a path to the portal target.
            var grid = GAME_STATE.Cells.ToDictionary(c => (c.X, c.Y), c => c.Content);
            var path = FindPath(ME.X, ME.Y, portalTarget.X, portalTarget.Y);
    ...
    public class OmniStriderNode
    {
        ...
        public OmniStriderNode? Parent;
        public int BlankStreak;
        public int PelletStreak;
        public BotAction? DirFromParent { get; }

        public OmniStriderNode(int x, int y, int g, int h, OmniStriderNode? parent, int tieBreak, int blankStreak, int pelletStreak, BotAction? dirFromParent)
        {
            X = x;
            Y = y;
            G = g;
    ...
    public class OmniStriderCluster
    {
        public List<Cell> Cells { get; set; }
        public float Score { get; set; }
        public Cell EntryPoint { get; set; }
        public List<OmniStriderNode>? Path { get; set; }
    }

    public sealed class OmniStriderZookeeperTracker
    {
        public Animal? Target;
        public List<OmniStriderNode>? Path;

...
namespace NETCoreBot.Strategy
{
    public static class Rabbit
    {
        ...
        public static List<RabbitNode>? PersistentPath { get; set; } = null;
        ...
        public static Dictionary<(int x, int y), int> ZOOKEEP_DISTANCE_MAP = new();
        ...
        public static RabbitCluster BestCluster { get; set; }
        ...
        public static BotCommand? ProcessState(GameState gameStateDTO, Guid id)
        {
            ...
            {
                ZOOKEEP_DISTANCE_MAP = MapZookeeperDistance();
                //GameLogger.LogCollection("CONTESTED_CELLS_MAP", CONTESTED_CELLS_MAP);
            }
        ...
        public static Dictionary<(int x, int y), int> MapZookeeperDistance()
        {
            var zk = GAME_STATE.Zookeepers.FirstOrDefault();
            var distMap = new Dictionary<(int, int), int>();
            ...
            if (zk != null)
            {
                ...
                while (queue.Count > 0)
                {
                    ...
                    foreach (var (nx, ny) in GetNeighborsZK(x, y))
                    {
                        if (!distMap.ContainsKey((nx, ny)) && IsWalkable(nx, ny))
                        {
                            distMap[(nx, ny)] = cost + 1;
            ...
            if (GAME_STAGE != GameStage.EarlyGame)
            {
                if (IsZookeeperNear())
                {
                    ...
                    if (HasPortals())
                    {
                        ...
                        var escape = EscapeViaPortalNewSZK(); // your existing routine – unchanged
                        if (escape != null)
                        {
        ...
        public static List<RabbitNode>? FindPathZookeeper(
            int startX,
            int startY,
            int targetX,
            int targetY,
            Dictionary<(int, int), CellContent> grid)
        {
            ...
            var openSet = new List<RabbitNode>();
            var closedSet = new HashSet<(int, int)>();

            var startNode = new RabbitNode(
                startX, startY,
                0,
        ...
        public static IEnumerable<RabbitNode> GetNeighborsZookeeper(
            RabbitNode current,
            Dictionary<(int, int), CellContent> grid,
            int targetX,
            int targetY)
        ...
        public static float ScoreCluster(RabbitCluster cluster)
        {
            if (cluster.Cells.Count == 0)
                return 0;

            // Estimate cluster reach cost as the min actual path distance (not Manhattan)
            ...
            return (float)(clusterSize - ALPHA * minDist);
        }
        ...
        public static RabbitCluster FindBestClusterEarlyGame()
        {
            ...
            var clusters = new List<RabbitCluster>();
            ...
            RabbitCluster bestCluster = clusters.OrderByDescending(c => c.Score).FirstOrDefault();
        ...
        public static RabbitCluster BFSClusterEarlyGame((int x, int y) start, HashSet<(int, int)> visited)
        {
        ...
        public static RabbitCluster FindBestClusterMidGame()
        {
            ...
            var fullPath = new List<RabbitNode>();
        ...
        public static RabbitCluster BFSClusterMidGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static List<RabbitNode> FindLongestPelletPathInCluster((int x, int y) start, HashSet<(int, int)> clusterSet) // ✅ Tune this externally
        {
            var bestPath = new List<RabbitNode>();
            var visited = new HashSet<(int, int)>();

            void DFS((int x, int y) current, List<RabbitNode> currentPath)
            {
                visited.Add(current);
                currentPath.Add(new RabbitNode(current.x, current.y, 0, 0, null, 0, 0, 0, null));

                // ✅ Always track the longest path seen so far
                if (currentPath.Count > bestPath.Count)
                    bestPath = new List<RabbitNode>(currentPath);
                ...
                {
                    if (clusterSet.Contains(neighbor) && !visited.Contains(neighbor) && !CONTESTED_CELLS_MAP.Contains(neighbor) && !CORRIDOR_CELLS_MAP.Contains(neighbor))
                    {
        ...
        public static RabbitCluster FindBestClusterLateGame()
        {
            var pelletCells = GAME_STATE.Cells.Where(c => c.Content == CellContent.Pellet && !CONTESTED_CELLS_MAP.Contains(new(c.X, c.Y)) && !CORRIDOR_CELLS_MAP.Contains(new(c.X, c.Y))).ToList();
        ...
        public static RabbitCluster BFSClusterLateGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static List<RabbitNode>? FindPath(int startX, int startY, int targetX, int targetY, bool ignorePortals = false)
        {
            // ─── data-structures ──────────────────────────────────────────────────────
            var open = new PriorityQueue<RabbitNode, int>();               // min-heap by F
            var openDict = new Dictionary<(int, int), RabbitNode>();           // coord → best node
        ...
        public static IEnumerable<RabbitNode> FindPathNeighbors(RabbitNode current, int targetX, int targetY, bool ignorePortals = false)
        {
            // (dx, dy, heading)
            ...
            foreach (var (dx, dy, dir) in dirs)
            {
                ...
                // 2.  bonus / penalty
                //--------------------------------------------------------------------
                int bonus = FindPathBonus(new RabbitNode(nx, ny, newG, 0, current, 0, nextBlank, nextStreak, null));
                int penalty = FindPathPenalty(current, new RabbitNode(nx, ny, newG, 0, current, 0, nextBlank, nextStreak, null));
        ...
        public static int FindPathBonus(RabbitNode node)
        {
            int bonus = 0;
        ...
        public static int FindPathPenalty(RabbitNode current, RabbitNode neighbor)
        {
            int penalty = 0;
        ...
        public static int GetDistanceFromZookeeper(int tx, int ty)
        {
            if (ZOOKEEP_DISTANCE_MAP.TryGetValue((tx, ty), out var d))
            {
                return d;
            }
            else
        ...
        public static List<RabbitNode> ReconstructPath(RabbitNode node)
        {
            var path = new List<RabbitNode>();
            while (node != null)
            {
                path.Add(node);
                node = node.Parent;
        ...
        public static IEnumerable<RabbitNode> GetNeighborsClean(RabbitNode current, int targetX, int targetY)
        {
            int[,] directions = new int[,] { { 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 } };
        ...
        public static double EvaluatePathScore(List<RabbitNode> path)
        {
            int pellets = CountPelletsOnPath(path);
            return pellets;
        }
        public static int CountPelletsOnPath(List<RabbitNode> path)
        {
            int count = 0;
        ...
        public static List<RabbitNode>? FindPathOpponent(int startX, int startY, int targetX, int targetY)
        {
            var openSet = new List<RabbitNode>();
            var closedSet = new HashSet<(int, int)>();
        ...
        public static IEnumerable<RabbitNode> FindPathNeighborsOpponent(RabbitNode current, int targetX, int targetY)
        {
            int[,] directions = new int[,] { { 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 } };
        ...
        private static readonly Dictionary<Guid, RabbitZookeeperTracker> _zkCache = new(); // NEW
        ...
        private static bool IsZookeeperNear()
        {
            foreach (var zk in GAME_STATE.Zookeepers)
            {
                ...
                //int distance = GetDistanceFromMe(zk.X, zk.Y);
                //if (distance <= SAFE_DISTANCE * 2) // 2x safe distance
                //{
                //    GameLogger.LogWatch("ZookeeperEvent", $"Zookeeper {zk.Id} is near at distance {distance}");

                //    var pathToMe = PathToAnimal(zk, ME);
                //    if (pathToMe != null && pathToMe.Count <= SAFE_DISTANCE)
                //    {
                //        return true;
                //    }
                ...
                //else
                //{
                //    GameLogger.LogWatch("ZookeeperEvent", $"Zookeeper {zk.Id} is far at distance {distance}");
                //}
            }
        ...
        private static int KeeperStepsToMe(Guid zkId)
        {
            ...
            if (!_zkCache.TryGetValue(zkId, out var trk))
                _zkCache[zkId] = trk = new RabbitZookeeperTracker();
        ...
        private static List<RabbitNode>? PathToAnimal(Zookeeper zk, Animal tgt)
            => FindPathKeeper(zk.X, zk.Y, tgt.X, tgt.Y);

        // ---------------------------------------------------------------- A*
        private static List<RabbitNode>? FindPathKeeper(int sx, int sy, int tx, int ty)
        {
            var open = new PriorityQueue<RabbitNode, int>();
            var best = new Dictionary<(int, int), int>();

            var start = new RabbitNode(sx, sy, 0,
        ...
        private static IEnumerable<RabbitNode> KeeperNeighbours(RabbitNode n, int tx, int ty)
        {
            foreach (var (dx, dy) in new (int, int)[] { (0, -1), (0, 1), (-1, 0), (1, 0) })
            {
                ...
                yield return new RabbitNode(nx, ny, n.G + 1,
        ...
        public static BotCommand? EscapeViaPortalNewSZK()
        {
            ...
            Zookeeper closestZookeeper = GAME_STATE.Zookeepers.First();


            // 3. Find a portal cell on the chosen edge (filtering using IsPortalCell).
            Cell? portalTarget = null;
            List<RabbitNode>? path = null;
            ...
            foreach (var pc in allPortalCellsOrdered)
            {
                var myDistance = GetDistanceFromMe(pc.X, pc.Y);
                var zkDistance = GetDistanceFromZookeeper(pc.X, pc.Y);

                //If zookeeper is closer than me, skip this portal cell
                if (zkDistance < myDistance)
                {
                    continue;
                }

                path = FindPath(ME.X, ME.Y, pc.X, pc.Y, true);
                var zkPath = FindPathKeeper(closestZookeeper.X, closestZookeeper.Y, ME.X, ME.Y);

                if (PathsIntersect(path, zkPath))
                {
                    //GameLogger.LogWatch("EscapeViaPortal", $"Skipping portal cell ({pc.X}, {pc.Y}) due to intersection with zookeeper path.");
                    continue; // Skip this portal cell if paths intersect
                }
                ...
                break; // Found a valid portal target
            }
            ...
            if (path.Count < 2)
            {
                ...
                if (escapeAction.HasValue)
                {
                    GameLogger.LogWatch("EscapeViaPortal", $"Tick: {GAME_STATE.Tick} | Escape move: {escapeAction.Value} (exiting portal)");
                    PortalEnabled = false;
                    PortalsLockedUntilTick = GAME_STATE.Tick + PORTAL_COOLDOWN; // Lock portals for 10 ticks after escape
                    return new BotCommand { Action = escapeAction.Value };
                }
                else
                {
        ...
        /// </summary>
        private static bool PathsIntersect(List<RabbitNode> pathA, List<RabbitNode> pathB)
        {
    ...
    public class RabbitNode
    {
        ...
        public RabbitNode? Parent;
        public int BlankStreak;
        public int PelletStreak;
        public BotAction? DirFromParent { get; }

        public RabbitNode(int x, int y, int g, int h, RabbitNode? parent, int tieBreak, int blankStreak, int pelletStreak, BotAction? dirFromParent)
        {
            X = x;
            Y = y;
            G = g;
    ...
    public class RabbitCluster
    {
        ...
        public List<RabbitNode>? Path { get; set; }
    }

    public sealed class RabbitZookeeperTracker
    {
        public Animal? Target;
        public List<RabbitNode>? Path;

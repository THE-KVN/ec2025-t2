...
namespace NETCoreBot.Strategy
{
    public static class Snake
    {
        ...
        public static List<SnakeNode>? PersistentPath { get; set; } = null;
        ...
        public static SnakeCluster BestCluster { get; set; }
        ...
        public static List<SnakeNode>? FindPathZookeeper(
            int startX,
            int startY,
            int targetX,
            int targetY,
            Dictionary<(int, int), CellContent> grid)
        {
            ...
            var openSet = new List<SnakeNode>();
            var closedSet = new HashSet<(int, int)>();

            var startNode = new SnakeNode(
                startX, startY,
                0,
        ...
        public static IEnumerable<SnakeNode> GetNeighborsZookeeper(
            SnakeNode current,
            Dictionary<(int, int), CellContent> grid,
            int targetX,
            int targetY)
        ...
        public static float ScoreCluster(SnakeCluster cluster)
        {
        ...
        public static SnakeCluster FindBestClusterEarlyGame()
        {
            ...
            var clusters = new List<SnakeCluster>();
            ...
            SnakeCluster bestCluster = clusters.OrderByDescending(c => c.Score).FirstOrDefault();
        ...
        public static SnakeCluster BFSClusterEarlyGame((int x, int y) start, HashSet<(int, int)> visited)
        {
        ...
        public static SnakeCluster FindBestClusterMidGame()
        {
            ...
            var fullPath = new List<SnakeNode>();
        ...
        public static SnakeCluster BFSClusterMidGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static List<SnakeNode> FindLongestPelletPathInCluster((int x, int y) start, HashSet<(int, int)> clusterSet) // ✅ Tune this externally
        {
            var bestPath = new List<SnakeNode>();
            var visited = new HashSet<(int, int)>();

            void DFS((int x, int y) current, List<SnakeNode> currentPath)
            {
                visited.Add(current);
                currentPath.Add(new SnakeNode(current.x, current.y, 0, 0, null, 0, 0, 0, null));

                // ✅ Always track the longest path seen so far
                if (currentPath.Count > bestPath.Count)
                    bestPath = new List<SnakeNode>(currentPath);
        ...
        public static SnakeCluster FindBestClusterLateGame()
        {
            var pelletCells = GAME_STATE.Cells.Where(c => c.Content == CellContent.Pellet && !CONTESTED_CELLS_MAP.Contains(new(c.X, c.Y)) && !CORRIDOR_CELLS_MAP.Contains(new(c.X, c.Y))).ToList();
        ...
        public static SnakeCluster BFSClusterLateGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static List<SnakeNode>? FindPath(int startX, int startY, int targetX, int targetY, bool ignorePortals = false)
        {
            // ─── data-structures ──────────────────────────────────────────────────────
            var open = new PriorityQueue<SnakeNode, int>();               // min-heap by F
            var openDict = new Dictionary<(int, int), SnakeNode>();           // coord → best node
        ...
        public static IEnumerable<SnakeNode> FindPathNeighbors(SnakeNode current, int targetX, int targetY, bool ignorePortals = false)
        {
            // (dx, dy, heading)
            ...
            foreach (var (dx, dy, dir) in dirs)
            {
                ...
                // 2.  bonus / penalty
                //--------------------------------------------------------------------
                int bonus = FindPathBonus(new SnakeNode(nx, ny, newG, 0, current, 0, nextBlank, nextStreak, null));
                int penalty = FindPathPenalty(current, new SnakeNode(nx, ny, newG, 0, current, 0, nextBlank, nextStreak, null));
        ...
        public static int FindPathBonus(SnakeNode node)
        {
            int bonus = 0;
        ...
        public static int FindPathPenalty(SnakeNode current, SnakeNode neighbor)
        {
            int penalty = 0;
        ...
        public static List<SnakeNode> ReconstructPath(SnakeNode node)
        {
            var path = new List<SnakeNode>();
            while (node != null)
            {
                path.Add(node);
                node = node.Parent;
        ...
        public static IEnumerable<SnakeNode> GetNeighborsClean(SnakeNode current, int targetX, int targetY)
        {
            int[,] directions = new int[,] { { 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 } };
        ...
        public static double EvaluatePathScore(List<SnakeNode> path)
        {
            int pellets = CountPelletsOnPath(path);
            return pellets;
        }
        public static int CountPelletsOnPath(List<SnakeNode> path)
        {
            int count = 0;
        ...
        public static List<SnakeNode>? FindPathOpponent(int startX, int startY, int targetX, int targetY)
        {
            var openSet = new List<SnakeNode>();
            var closedSet = new HashSet<(int, int)>();
        ...
        public static IEnumerable<SnakeNode> FindPathNeighborsOpponent(SnakeNode current, int targetX, int targetY)
        {
            int[,] directions = new int[,] { { 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 } };
        ...
        private static readonly Dictionary<Guid, SnakeZookeeperTracker> _zkCache = new(); // NEW
        ...
        private static int KeeperStepsToMe(Guid zkId)
        {
            ...
            if (!_zkCache.TryGetValue(zkId, out var trk))
                _zkCache[zkId] = trk = new SnakeZookeeperTracker();
        ...
        private static List<SnakeNode>? PathToAnimal(Zookeeper zk, Animal tgt)
            => FindPathKeeper(zk.X, zk.Y, tgt.X, tgt.Y);

        // ---------------------------------------------------------------- A*
        private static List<SnakeNode>? FindPathKeeper(int sx, int sy, int tx, int ty)
        {
            var open = new PriorityQueue<SnakeNode, int>();
            var best = new Dictionary<(int, int), int>();

            var start = new SnakeNode(sx, sy, 0,
        ...
        private static IEnumerable<SnakeNode> KeeperNeighbours(SnakeNode n, int tx, int ty)
        {
            foreach (var (dx, dy) in new (int, int)[] { (0, -1), (0, 1), (-1, 0), (1, 0) })
            {
                ...
                yield return new SnakeNode(nx, ny, n.G + 1,
        ...
        /// </summary>
        private static bool PathsIntersect(List<SnakeNode> pathA, List<SnakeNode> pathB)
        {
    ...
    public class SnakeNode
    {
        ...
        public SnakeNode(int x, int y, int g, int h, SnakeNode? parent, int tieBreak, int blankStreak, int pelletStreak, BotAction? dirFromParent)
        {
            X = x;
            Y = y;
            G = g;
    ...
    public class SnakeCluster
    {
        public List<Cell> Cells { get; set; }
        public float Score { get; set; }
        public Cell EntryPoint { get; set; }
        public List<SnakeNode>? Path { get; set; }
    }

    public sealed class SnakeZookeeperTracker
    {

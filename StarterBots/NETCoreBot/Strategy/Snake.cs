...
namespace NETCoreBot.Strategy
{
    public static class Snake
    {
        ...
        public static int VISITED_TILE_PENALTY_FACTOR = 25;
        public static double REVERSAL_RANDOM_SKIP_CHANCE = 0.5;
        public static int REVERSING_TO_PARENT_PENALTY = 1;
        public static double ZOOKEEPER_AVOIDANCE_FACTOR = 25;
        public static int DANGER_THRESHOLD = 50;
        public static int SAFE_DISTANCE = 5;
        public static int REPLAN_INTERVAL = 3;
        public static int CORRIDOR_PENALTY = 25;

        //additional
        public static int EMPTY_CELL_PENALTY = 1;
        public const int PREDICTIVE_BLOCKING_PENALTY = 20;
        ...
        public static Guid myAnimalId = Guid.Empty;
        ...
        public static bool IsPathStillValid(List<SnakeNode>? path, Animal myAnimal, List<Cell> allCells, GameState state)
        {
        ...
        public static List<SnakeNode>? ValidateOrComputePath(Animal myAnimal, Cell target, Dictionary<(int, int), CellContent> grid, GameState state)
        {
        ...
        public static void LogCurrentPath(List<SnakeNode> path)
        {
            var coords = path.Select(n => $"({n.X},{n.Y})");
            Console.WriteLine("Current path: " + string.Join(" -> ", coords));
        }

        public static BotAction? ComputeNextMoveRateLimited(Animal myAnimal, List<SnakeNode> path)
        {
        ...
        public static List<SnakeNode>? FindPath(
            int startX,
            int startY,
            int targetX,
            int targetY,
        ...
        {
            // If skipPrediction is true, use an empty set
            var predictedCells = skipPrediction ? new HashSet<(int, int)>() : PredictOccupiedCellsNextTurn(state);
            ...
            while (openSet.Count > 0)
            {
                ...
                // Pass the predictedCells set to the neighbor generator.
                foreach (var neighbor in GetNeighborsPortal(current, grid, targetX, targetY, myAnimal, state, predictedCells))
                {
        ...
        public static IEnumerable<SnakeNode> GetNeighborsPortal(
    SnakeNode current,
    Dictionary<(int, int), CellContent> grid,
    int targetX,
    int targetY,
    Animal myAnimal,
    GameState state,
    HashSet<(int, int)> predictedCells) // New parameter
        {
            ...
            for (int i = 0; i < directions.GetLength(0); i++)
            {
                ...
                if (content == CellContent.Empty)
                    additionalPenalty += EMPTY_CELL_PENALTY;

                // New: Add predictive blocking penalty if this cell is predicted to be occupied.
                if (predictedCells.Contains((nx, ny)))
                    additionalPenalty += PREDICTIVE_BLOCKING_PENALTY;
                ...
                // Determine if this neighbor is a reversal move (stutter)
                bool isStutter = (current.Parent != null && nx == current.Parent.X && ny == current.Parent.Y);

                // Use the new heuristic function.
                CellContent targetContent = grid.TryGetValue((targetX, targetY), out var targetCont) ? targetCont : CellContent.Empty;
                int newH = ComputeHeuristic(nx, ny, targetX, targetY, content, targetContent, isStutter);
        ...
        public static int ComputeHeuristic(
    int currentX, int currentY,
    int targetX, int targetY,
    CellContent currentContent, CellContent targetContent,
    bool isStutter)
        {
            int baseH = Manhattan(currentX, currentY, targetX, targetY);
            return baseH;
        }



        // Predict the next occupied cells for all animals in the game state.
        public static HashSet<(int, int)> PredictOccupiedCellsNextTurn(GameState state)
        {
            var predictedCells = new HashSet<(int, int)>();

            // For each animal in the game state
            foreach (var animal in state.Animals)
            {
                // Optional: Skip your own animal if desired.
                if (animal.Id == myAnimalId) continue;

                // Find the nearest pellet for this animal.
                var pellet = FindNearestPelletForAnimal(animal, state);
                if (pellet != null)
                {
                    // Build the grid for pathfinding.
                    var grid = state.Cells.ToDictionary(c => (c.X, c.Y), c => c.Content);
                    // Call FindPath with skipPrediction set to true to avoid recursion.
                    var path = FindPath(animal.X, animal.Y, pellet.X, pellet.Y, grid, animal, state, true);
                    if (path != null && path.Count > 1)
                    {
                        // The second node represents the next move.
                        predictedCells.Add((path[1].X, path[1].Y));
                    }
                }
            }
        ...
        // Helper: Find the nearest pellet for a given animal.
        public static Cell? FindNearestPelletForAnimal(Animal animal, GameState state)
        {
            var pellets = state.Cells.Where(c => c.Content == CellContent.Pellet).ToList();
            if (!pellets.Any()) return null;
            return pellets.OrderBy(c => Manhattan(animal.X, animal.Y, c.X, c.Y)).First();
        }
    ...
    public class SnakeNode
    {
        ...
        public SnakeNode(int x, int y, int g, int h, SnakeNode? parent, int tieBreak = 0)
        {
            X = x;
            Y = y;
            G = g;

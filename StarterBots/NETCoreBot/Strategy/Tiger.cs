...
namespace NETCoreBot.Strategy
{
    public static class Tiger
    {
        ...
        public static TigerCluster BestCluster { get; set; }
        ...
        public static List<TigerNode>? FindPathZookeeper(
            int startX,
            int startY,
            int targetX,
            int targetY,
        ...
        public static IEnumerable<TigerNode> GetNeighborsZookeeper(
            TigerNode current,
            Dictionary<(int, int), CellContent> grid,
            int targetX,
            int targetY)
        ...
        public static float ScoreCluster(TigerCluster cluster)
        {
        ...
        public static TigerCluster FindBestClusterEarlyGame()
        {
            ...
            var clusters = new List<TigerCluster>();
            ...
            TigerCluster bestCluster = clusters.OrderByDescending(c => c.Score).FirstOrDefault();
            ...
            var fullPath = new List<TigerNode>();
        ...
        public static TigerCluster BFSClusterEarlyGame((int x, int y) start, HashSet<(int, int)> visited)
        {
        ...
        public static TigerCluster FindBestClusterMidGame()
        {
        ...
        public static TigerCluster BFSClusterMidGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static List<TigerNode> FindLongestPelletPathInCluster((int x, int y) start, HashSet<(int, int)> clusterSet) // ✅ Tune this externally
        {
            var bestPath = new List<TigerNode>();
            var visited = new HashSet<(int, int)>();

            void DFS((int x, int y) current, List<TigerNode> currentPath)
            {
                visited.Add(current);
                currentPath.Add(new TigerNode(current.x, current.y, 0, 0, null, 0, 0, 0, null));

                // ✅ Always track the longest path seen so far
                if (currentPath.Count > bestPath.Count)
                    bestPath = new List<TigerNode>(currentPath);
        ...
        public static TigerCluster FindBestClusterLateGame()
        {
            var pelletCells = GAME_STATE.Cells.Where(c => c.Content == CellContent.Pellet && !CONTESTED_CELLS_MAP.Contains(new(c.X, c.Y)) && !CORRIDOR_CELLS_MAP.Contains(new(c.X, c.Y))).ToList();
        ...
        public static TigerCluster BFSClusterLateGame((int x, int y) start, HashSet<(int, int)> visited)
        {
            var cluster = new List<Cell>();
            var queue = new Queue<(int x, int y)>();
            queue.Enqueue(start);
        ...
        public static List<TigerNode>? FindPath(int startX, int startY, int targetX, int targetY, bool ignorePortals = false)
        {
            // ─── data-structures ──────────────────────────────────────────────────────
            var open = new PriorityQueue<TigerNode, int>();               // min-heap by F
            var openDict = new Dictionary<(int, int), TigerNode>();           // coord → best node
        ...
        public static IEnumerable<TigerNode> FindPathNeighbors(TigerNode current, int targetX, int targetY, bool ignorePortals = false)
        {
            // (dx, dy, heading)
            ...
            foreach (var (dx, dy, dir) in dirs)
            {
                ...
                // 2.  bonus / penalty
                //--------------------------------------------------------------------
                int bonus = FindPathBonus(new TigerNode(nx, ny, newG, 0, current, 0, nextBlank, nextStreak, null));
                int penalty = FindPathPenalty(current, new TigerNode(nx, ny, newG, 0, current, 0, nextBlank, nextStreak, null));
        ...
        public static int FindPathBonus(TigerNode node)
        {
            int bonus = 0;
        ...
        public static int FindPathPenalty(TigerNode current, TigerNode neighbor)
        {
            int penalty = 0;
        ...
        public static List<TigerNode> ReconstructPath(TigerNode node)
        {
        ...
        public static IEnumerable<TigerNode> GetNeighborsClean(TigerNode current, int targetX, int targetY)
        {
            int[,] directions = new int[,] { { 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 } };
        ...
        public static double EvaluatePathScore(List<TigerNode> path)
        {
            int pellets = CountPelletsOnPath(path);
            return pellets;
        }
        public static int CountPelletsOnPath(List<TigerNode> path)
        {
            int count = 0;
        ...
        public static List<TigerNode>? FindPathOpponent(int startX, int startY, int targetX, int targetY)
        {
            var openSet = new List<TigerNode>();
            var closedSet = new HashSet<(int, int)>();
        ...
        public static IEnumerable<TigerNode> FindPathNeighborsOpponent(TigerNode current, int targetX, int targetY)
        {
            int[,] directions = new int[,] { { 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 } };
        ...
        private static readonly Dictionary<Guid, TigerZookeeperTracker> _zkCache = new(); // NEW
        ...
        private static int KeeperStepsToMe(Guid zkId)
        {
            ...
            if (!_zkCache.TryGetValue(zkId, out var trk))
                _zkCache[zkId] = trk = new TigerZookeeperTracker();
        ...
        private static List<TigerNode>? PathToAnimal(Zookeeper zk, Animal tgt)
            => FindPathKeeper(zk.X, zk.Y, tgt.X, tgt.Y);

        // ---------------------------------------------------------------- A*
        private static List<TigerNode>? FindPathKeeper(int sx, int sy, int tx, int ty)
        {
            var open = new PriorityQueue<TigerNode, int>();
            var best = new Dictionary<(int, int), int>();

            var start = new TigerNode(sx, sy, 0,
        ...
        private static IEnumerable<TigerNode> KeeperNeighbours(TigerNode n, int tx, int ty)
        {
            foreach (var (dx, dy) in new (int, int)[] { (0, -1), (0, 1), (-1, 0), (1, 0) })
            {
                ...
                yield return new TigerNode(nx, ny, n.G + 1,
        ...
        /// </summary>
        private static bool PathsIntersect(List<TigerNode> pathA, List<TigerNode> pathB)
        {
    ...
    public class TigerNode
    {
        ...
        public TigerNode(int x, int y, int g, int h, TigerNode? parent, int tieBreak, int blankStreak, int pelletStreak, BotAction? dirFromParent)
        {
            X = x;
            Y = y;
            G = g;
    ...
    public class TigerCluster
    {
        public List<Cell> Cells { get; set; }
        public float Score { get; set; }
        public Cell EntryPoint { get; set; }
        public List<TigerNode>? Path { get; set; }
    }

    public sealed class TigerZookeeperTracker
    {
        public Animal? Target;
        public List<TigerNode>? Path;
